...
value<-round(c(ts1_cast$mean,ts2_cast$mean*100,ts3_cast$mean,ts4_cast$mean),4)
last<-round(c(last(s1),last(s2)*100,last(s3), last(s4)),4)
forecast<-data.frame(parameter, last, value)#%, unit)
colnames(forecast) <- c("Parameter", "last","Fcast")#, "Desc")
```
